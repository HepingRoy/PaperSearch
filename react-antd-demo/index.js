    	  ...
    	  CALLJSON(url,data,function (e) {
              if(e.code==1){
                  ...
                  for(var i =0;i<e.rows.length;i++){
                      var ob = {key:i,data:{url:e.rows[i]["url"],name:e.rows[i]["title"],label:e.rows[i]["label"],content:e.rows[i]["content"],type:(e.rows[i]["title"]).split(".")[1]}};
                      dataSource.push(ob);
                  }
    ...
    render: function () {
        ...
        var columns = [{
            ...
            render: function (text, record, index) {
                var lab = (this.state.data[index]['data']['label']).substring(1,(this.state.data[index]['data']['label']).length-1).replace(/,/g, " | ");
                var filekey ="/solrsearch/solr/download.do?fileId="+encodeURI(encodeURI(this.state.data[index]['data']['url']));
                var pic ="javascript:pic('"+this.state.data[index]['data']['name']+"')";
            	return (<div>
                    <Row>
                        <Col>
